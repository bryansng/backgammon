...
public class TeaCup implements BotAPI {
	...
	private double pipCountWeight, blockBlotWeight, homeBlockWeight, primingWeight, anchorWeight, escapedCheckersWeight,
			checkersInHomeWeight, checkersTakenOffWeight, pipsCoveredWeight;
	...
	/**
	 * � Generate a list of all possible (i.e. legal) plays � Generate all possible
	 * resulting board positions, i.e. position AFTER each play has been made � Go
	 * through the board positions and score to them � The score should reflect the
	 * probability that the bot will win from that position. � In reality, that is
	 * very hard to calculate, so the score should be an estimate of how good the
	 * position is. � The bot should select the play with the highest score (i.e.
	 * the highest probability of the bot winning).
	 */
	...
	private double getScore(int[] c, int[] o) {
		...
		if (isOpposedBearOff(c, o)) {
			...
			score = pipCountDiff(c, o) + blockBlotDiff(c, o) + numHomeBoardBlocks(c, o) + lengthPrimeCapturedChecker(c, o)
					+ anchor(c, o) + numEscapedCheckers(c, o) + numCheckersInHomeBoard(c, o) + numCheckersTakenOff(c, o)
					+ numPipsCovered(c, o);
		}
		if (DEBUG)
	...
	private double pipCountDiff(int[] c, int[] o) {
		...
		/*
		 * System.out.println("cTotal: " + cTotal); System.out.println("oTotal: " +
		 * oTotal);
		 */
	...
	private double blockBlotDiff(int[] c, int[] o) {
		...
		/*
		 * System.out.println("cBlocks: " + cBlocks); System.out.println("oBlots: " +
		 * oBlots);
		 */
	...
	private double numHomeBoardBlocks(int[] c, int[] o) {
		...
		for (int i = 1; i <= MAX; i++) {
			if (c[i] > 1)
				num += i; // take into consideration importance.
		}
		/*
	...
	private double lengthPrimeCapturedChecker(int[] c, int[] o) {
		...
		//
		// length of prime with a threshold of 4 (put 3 below, but will register after
		// 3, so 4).
		 ...
		 * System.out.println("Index of last Prime: " + (maxLen > 3 ? indexOfLastPrime :
		 * -1)); System.out.println("Captured: " + captured);
		 */
	...
	private double anchor(int[] c, int[] o) {
		...
		for (int i = 19; i <= 24; i++) {
			if (c[i] > 1)
				num += c[i]; // take into consideration importance.
		}
		/*
	...
	private double numEscapedCheckers(int[] c, int[] o) {
		...
		/*
		 * System.out.println("Last checker index: " + (lastCheckerIndex == -1 ? 0 :
		 * lastCheckerIndex)); System.out.println("Num checkers escaped: " + num);
		 */
	...
	private int getBestMove(double[] probs) {
		...
		if (DEBUG)
			System.out.println("Chosen index: " + maxAt);
		return maxAt;
	}
	...
	private String considerOfferingDoubleDice(PlayerAPI currentPlayer, PlayerAPI opponentPlayer) {
		...
		if ((currentPlayer.getScore() == match.getLength() - 2) && (opponentPlayer.getScore() == match.getLength() - 2)) {
			if (meWinningChance >= 0 && meWinningChance <= .50)
				decision = "y";
			else if (meWinningChance >= .50 && meWinningChance <= 0.75)
				decision = "y";
